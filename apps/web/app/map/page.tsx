      ...
      if (!isNaN(latNum) && !isNaN(lngNum)) {
        ...
        if (savesAtLocation.length > 1) {
          ...
          const surfInfo: SurfInfo = {
            ...
            derivedMetrics: {
              BEGINNER: { surfScore: matchingSaved.surfScore, surfGrade: matchingSaved.surfGrade, surfGradeNumeric: 0 },
              INTERMEDIATE: { surfScore: matchingSaved.surfScore, surfGrade: matchingSaved.surfGrade, surfGradeNumeric: 0 },
    ...
    try {
      ...
      if (query) {
        const [response] = await Promise.all([
          surfService.searchSpots(query, { ...utcOptions, multi: true, language: locale === 'ko' ? 'ko' : undefined }),
  ...
  const handleSaveSpot = async (surfInfo: SurfInfo) => {
    ...
    try {
      await saveItem({
        ...
        surfGrade: getMetricsForLevel(surfInfo.derivedMetrics, effectiveLevel).surfGradeNumeric,
        address: displayName,
