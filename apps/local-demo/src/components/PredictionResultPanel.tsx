...
import { getGradeBgColor, getGradeTextColor, getGradeBorderColor, parseUTCTimestamp, getSurfScoreColors } from '../services/surfInfoService';
...
export default function PredictionResultPanel({
  ...
  const dateStr = parsedDate
    ? `${parsedDate.getUTCFullYear()}-${(parsedDate.getUTCMonth() + 1).toString().padStart(2, '0')}-${parsedDate.getUTCDate().toString().padStart(2, '0')}`
    : '';
  ...
  return (
    <div
      className={`
        ...
        md:bottom-0 md:animate-none md:rounded-none md:right-auto md:left-0 md:max-h-none md:w-96
      ...
      <div className="flex-1 p-6 space-y-8 overflow-y-auto">
        ...
        <div className="bg-indigo-50 rounded-xl p-4 text-center border border-indigo-100">
          <p className="text-xs text-indigo-500">
            {locale === 'ko'
              ? '⚡ 데모 예측 — 실제 AI 모델 대신 시뮬레이션된 예측 데이터입니다'
              : '⚡ Demo prediction — simulated forecast data, not a live AI model'}
          </p>
        </div>
      </div>
    </div>
